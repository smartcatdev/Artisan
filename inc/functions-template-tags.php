 ...
 * @package Zenith
 */
if ( !function_exists( 'zenith_posted_on' ) ) :
    ...
    function zenith_posted_on() {
        ...
        $posted_on = sprintf(
                ...
                esc_html_x( 'Posted on %s', 'post date', 'zenith' ), '<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
        );
...
if ( !function_exists( 'zenith_posted_by' ) ) :
    ...
    function zenith_posted_by() {
        $byline = sprintf(
                ...
                esc_html_x( 'by %s', 'post author', 'zenith' ), '<span class="author vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author() ) . '</a></span>'
        );
...
if ( !function_exists( 'zenith_entry_footer' ) ) :

    /**
     * Prints HTML with meta information for the categories, tags and comments.
     */
    function zenith_entry_footer() {
        // Hide category and tag text for pages.
        if ( 'post' === get_post_type() ) {
            /* translators: used between list items, there is a space after the comma */
            $categories_list = get_the_category_list( esc_html__( ', ', 'zenith' ) );
            if ( $categories_list ) {
                /* translators: 1: list of categories. */
                printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'zenith' ) . '</span>', $categories_list ); // WPCS: XSS OK.
            }

            /* translators: used between list items, there is a space after the comma */
            $tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'zenith' ) );
            if ( $tags_list ) {
                /* translators: 1: list of tags. */
                printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'zenith' ) . '</span>', $tags_list ); // WPCS: XSS OK.
            }
        }
            ...
            comments_popup_link(
                    sprintf(
                            wp_kses(
                                    ...
                                    __( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'zenith' ), array (
                'span' => array (
                    'class' => array (),
                ),
                                    )
        ...
        edit_post_link(
                sprintf(
                        wp_kses(
                                ...
                                __( 'Edit <span class="screen-reader-text">%s</span>', 'zenith' ), array (
            'span' => array (
                'class' => array (),
            ),
                                )
...
if ( !function_exists( 'zenith_post_thumbnail' ) ) :
     ...
     */
    function zenith_post_thumbnail() {
...
if ( !function_exists( 'zenith_pro\zenith_render_designer' ) ) :
    
    add_action( 'zenith_designer', 'zenith_render_designer', 10 );
    function zenith_render_designer() { ?>

        <span class="zenith_by">
            <?php esc_html_e( 'Design by', 'zenith' ); ?>
        </span>
 ...
 * @return void
 */
function zenith_render_masonry_wrap_open() { ?>
    ...
    <div class="masonry-card-blog">
        ...
        <div class="container">
            ...
            <div class="row">
                ...
                <div class="<?php echo zenith_is_single_sidebar_active( 'blog' ) ? 'col-sm-9 col-md-9 col-lg-9' : 'col-sm-12'; ?>">
...
<?php }
add_action( 'zenith_blog_masonry_wrap_open', 'zenith_render_masonry_wrap_open');               
 ...
 * @return void
 */
function zenith_render_masonry_wrap_close() { ?>
                        
                    </div>
                    
                    <div class="zenith-pagination-links">
                        <?php the_posts_pagination( array( 'mid_size' => 1 ) ); ?>
                    </div>

                </div>
                
                <?php zenith_output_side_sidebar( 'blog', 'right' ) ?>
...
<?php }
add_action( 'zenith_blog_masonry_wrap_close', 'zenith_render_masonry_wrap_close');        
 ...
 * @return void
 */
function zenith_render_mosaic_wrap_open() { ?>
...
<?php }
add_action( 'zenith_blog_mosaic_wrap_open', 'zenith_render_mosaic_wrap_open'); 
 ...
 * @return void
 */
function zenith_render_mosaic_wrap_close() { ?>
...
<?php }
add_action( 'zenith_blog_mosaic_wrap_close', 'zenith_render_mosaic_wrap_close');        
 ...
 * @return void
 */
function zenith_render_standard_wrap_open() { ?>
...
<?php }
add_action( 'zenith_blog_standard_wrap_open', 'zenith_render_standard_wrap_open'); 
 ...
 * @return void
 */
function zenith_render_standard_wrap_close() { ?>
...
<?php }
add_action( 'zenith_blog_standard_wrap_close', 'zenith_render_standard_wrap_close');        
 ...
 */
function zenith_output_side_sidebar( $template = 'single', $location = 'right' ) {
    ...
    if ( $template == 'single' ) :
        ...
        if ( get_post_meta( get_the_ID(), ZENITH_META::SIDEBAR_TEMPLATE, true ) != 'none' && get_post_meta( get_the_ID(), ZENITH_META::SIDEBAR_LOCATION, true ) == 'sidebar-' . $location ) :

            if ( is_active_sidebar( get_post_meta( get_the_ID(), ZENITH_META::SIDEBAR_TEMPLATE, true ) ) ) : ?>

                <div class="col-sm-3 col-md-3 col-lg-3">

                    <div class="zenith-landr-sidebar-wrap single <?php echo esc_attr( $location ); ?>">

                        <?php dynamic_sidebar( get_post_meta( get_the_ID(), ZENITH_META::SIDEBAR_TEMPLATE, true ) ); ?>
        ...
        if ( is_active_sidebar( 'sidebar-blog-side' ) ) : ?>
            ...
            <div class="col-sm-3 col-md-3 col-lg-3">
                ...
                <div class="zenith-landr-sidebar-wrap archive <?php echo esc_attr( $location ); ?>">
 ...
 * @return void
 */
function zenith_render_edd_masonry_wrap_open() { ?>
...
<?php }
add_action( 'zenith_edd_masonry_wrap_open', 'zenith_render_edd_masonry_wrap_open');               
 ...
 * @return void
 */
function zenith_render_edd_masonry_wrap_close() { ?>
                ...
                <?php if ( is_active_sidebar( 'sidebar-shop' ) ) : ?>
                    ...
                    <div class="col-sm-3 col-md-3 col-lg-3">
                        ...
                        <div class="zenith-landr-sidebar-wrap edd-shop">
...
<?php }
add_action( 'zenith_edd_masonry_wrap_close', 'zenith_render_edd_masonry_wrap_close');

function zenith_output_edd_product_gallery( $gallery = null, $location = 'desktop' ) {
